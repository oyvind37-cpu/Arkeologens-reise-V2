...
const game=$("game"),chamber=$("chamber"),journal=$("journal"),desert=$("desert"),worldMap=$("worldMap"),journalQuickBtn=$("journalQuickBtn"),world=$("world"),aurora=$("aurora"),dialogText=$("dialogText");
...
const state={x:180,left:false,right:false,jumping:false,stage:0,vision:false,camera:0,idle:0,last:0,nearEye:false,eyeOpened:false,floorHintShown:false,arrivalLogged:false};

/* ---------- AURORA MOTION SYSTEM v5.8.1 / COHESIVE 2D MOTION ----------
   v5.8.1 keeps Aurora's approved visual model and retains the layered artwork, but
   deliberately reduces joint travel. The production character now moves as one
   connected body; the separate 3D Aurora Rig Lab is used for larger movement work. */
let mainVelocity=0;
let nextAmbientMotion=performance.now()+4200;
const motionGSAP=window.gsap||null;
const auroraMotionControllers=new WeakMap();
if(motionGSAP)document.documentElement.classList.add("gsap-ready","aurora-rig-ready");
...
class AuroraMotionController{
  constructor(el){
    ...
    this.head=this.sprite?.querySelector(".aurora-head");
    this.torso=this.sprite?.querySelector(".aurora-torso");
    this.armL=this.sprite?.querySelector(".aurora-arm-left");
    this.armR=this.sprite?.querySelector(".aurora-arm-right");
    this.legL=this.sprite?.querySelector(".aurora-leg-left");
    this.legR=this.sprite?.querySelector(".aurora-leg-right");
    this.parts=[this.head,this.torso,this.armL,this.armR,this.legL,this.legR].filter(Boolean);
    this.walkTl=null;this.idleTl=null;this.actionTl=null;this.turnTl=null;
    this.walking=false;this.actionBusy=false;
  ...
  neutralParts(duration=.18){
    if(!motionGSAP)return;
    if(this.parts.length)motionGSAP.to(this.parts,{x:0,y:0,rotation:0,scaleX:1,scaleY:1,duration,ease:"power2.out",overwrite:"auto"});
    if(this.sprite)motionGSAP.to(this.sprite,{x:0,y:0,rotation:0,scaleX:1,scaleY:1,duration,ease:"power2.out",overwrite:"auto"});
  ...
  startIdle(){
    ...
    if(this.torso)this.idleTl.to(this.torso,{y:-1.0,scaleY:1.007,rotation:.18,duration:1.9},0);
    if(this.head)this.idleTl.to(this.head,{y:-1.5,rotation:-.55,duration:1.9},0);
    if(this.armL)this.idleTl.to(this.armL,{rotation:.8,duration:1.9},0);
    if(this.armR)this.idleTl.to(this.armR,{rotation:-.65,duration:1.9},0);
    if(this.sprite)this.idleTl.to(this.sprite,{y:-.55,duration:1.9},0);
    if(this.shadow)this.idleTl.to(this.shadow,{scaleX:.976,opacity:.375,duration:1.9},0);
    if(this.head)this.idleTl.to(this.head,{rotation:.7,y:-.4,duration:1.55},1.9);
    if(this.torso)this.idleTl.to(this.torso,{rotation:-.14,scaleY:1.002,y:-.2,duration:1.55},1.9);
    if(this.armL)this.idleTl.to(this.armL,{rotation:-.45,duration:1.55},1.9);
    if(this.armR)this.idleTl.to(this.armR,{rotation:.55,duration:1.55},1.9);
  }
  walk(speed=1){
    ...
    if(!this.walkTl){
      ...
      if(this.legL)this.walkTl.to(this.legL,{rotation:9.5,x:-.5,y:-1,duration:.22},0);
      if(this.legR)this.walkTl.to(this.legR,{rotation:-9.5,x:.5,y:1,duration:.22},0);
      if(this.armL)this.walkTl.to(this.armL,{rotation:-7.5,y:-.5,duration:.22},0);
      if(this.armR)this.walkTl.to(this.armR,{rotation:7.5,y:.5,duration:.22},0);
      if(this.torso)this.walkTl.to(this.torso,{rotation:-.7,y:-1.5,duration:.22},0);
      if(this.head)this.walkTl.to(this.head,{rotation:.8,y:-1.9,duration:.22},0);
      if(this.sprite)this.walkTl.to(this.sprite,{y:-1.4,duration:.22},0);
      if(this.shadow)this.walkTl.to(this.shadow,{scaleX:.92,scaleY:1.04,opacity:.44,duration:.22},0);

      if(this.legL)this.walkTl.to(this.legL,{rotation:-9.5,x:.5,y:1,duration:.22},.22);
      if(this.legR)this.walkTl.to(this.legR,{rotation:9.5,x:-.5,y:-1,duration:.22},.22);
      if(this.armL)this.walkTl.to(this.armL,{rotation:7.5,y:.5,duration:.22},.22);
      if(this.armR)this.walkTl.to(this.armR,{rotation:-7.5,y:-.5,duration:.22},.22);
      if(this.torso)this.walkTl.to(this.torso,{rotation:.7,y:-1.0,duration:.22},.22);
      if(this.head)this.walkTl.to(this.head,{rotation:-.8,y:-1.4,duration:.22},.22);
      if(this.sprite)this.walkTl.to(this.sprite,{y:0,duration:.22},.22);
      if(this.shadow)this.walkTl.to(this.shadow,{scaleX:1.06,scaleY:.9,opacity:.35,duration:.22},.22);
    }
    this.walking=true;
    this.walkTl.timeScale(Math.max(.78,Math.min(1.55,speed))).play();
  }
  stop(){
    ...
    this.killIdle();
    const targets=this.parts.length?this.parts:[this.sprite];
    motionGSAP.timeline({onComplete:()=>this.startIdle()})
      .to(targets,{rotation:0,x:0,y:0,scaleX:1,scaleY:1,duration:.16,ease:"power2.out",overwrite:"auto"})
      .to(this.sprite,{y:1.8,rotation:-.45,duration:.08,ease:"power1.out"},0)
      .to(this.sprite,{y:0,rotation:0,duration:.16,ease:"back.out(1.5)"},.08);
    if(this.shadow)motionGSAP.to(this.shadow,{scaleX:1,scaleY:1,opacity:.42,duration:.2,ease:"power2.out"});
  }
  turn(left){
    const desired=left?"left":"right";
    if(this.facing===desired){this.el.classList.toggle("face-left",left);return}
    this.facing=desired;this.el.dataset.facing=desired;
    ...
    this.turnTl=motionGSAP.timeline({onComplete:()=>{this.turnTl=null;if(!this.walking&&!this.actionBusy)this.startIdle()}})
      .to(this.sprite,{scaleX:.18,y:1,duration:.10,ease:"power2.in"})
      .add(()=>this.el.classList.toggle("face-left",left))
      .to(this.sprite,{scaleX:1,y:0,duration:.14,ease:"power2.out"});
  }
  action(name,duration=1200){
    ...
    motionGSAP.set(this.parts,{overwrite:true});
    const done=()=>{this.actionBusy=false;this.actionTl=null;this.el.classList.remove(name);this.neutralParts(.16);setTimeout(()=>this.startIdle(),170)};
    const tl=this.actionTl=motionGSAP.timeline({onComplete:done});
    tl.to(this.parts,{rotation:0,x:0,y:0,scaleX:1,scaleY:1,duration:.10,ease:"power1.out"},0);
    if(name==="look-around"){
      if(this.head)tl.to(this.head,{rotation:-6.5,x:-1.2,duration:.34,ease:"power2.inOut"},.1)
        .to(this.head,{rotation:7,x:1.3,duration:.48,ease:"power2.inOut"})
        .to(this.head,{rotation:0,x:0,duration:.32,ease:"power2.out"});
      if(this.torso)tl.to(this.torso,{rotation:-.8,duration:.34},.1).to(this.torso,{rotation:.8,duration:.48}).to(this.torso,{rotation:0,duration:.32});
    }else if(name==="bend"){
      if(this.torso)tl.to(this.torso,{rotation:7.5,y:5,scaleY:.985,duration:.3,ease:"power2.inOut"},.1);
      if(this.head)tl.to(this.head,{rotation:-5,y:7,duration:.3,ease:"power2.inOut"},.1);
      if(this.legL)tl.to(this.legL,{rotation:3.5,y:3,duration:.3},.1);
      if(this.legR)tl.to(this.legR,{rotation:-3.5,y:3,duration:.3},.1);
      if(this.armR)tl.to(this.armR,{rotation:-18,x:2,y:4,duration:.34,ease:"power2.inOut"},.16);
      tl.to({}, {duration:Math.max(.18,duration/1000-.72)});
      tl.to(this.parts,{rotation:0,x:0,y:0,scaleX:1,scaleY:1,duration:.3,ease:"power2.out"});
    }else if(name==="reach-object"){
      if(this.torso)tl.to(this.torso,{rotation:2.6,x:1.5,duration:.28,ease:"power2.inOut"},.1);
      if(this.head)tl.to(this.head,{rotation:-2.4,x:1.5,duration:.28,ease:"power2.inOut"},.1);
      if(this.armR)tl.to(this.armR,{rotation:-29,x:5,y:-1,duration:.34,ease:"power2.inOut"},.1)
        .to(this.armR,{rotation:-25,x:6,y:-2,duration:.2,ease:"sine.inOut"})
        .to(this.armR,{rotation:0,x:0,y:0,duration:.28,ease:"power2.out"});
      tl.to([this.torso,this.head].filter(Boolean),{rotation:0,x:0,y:0,duration:.28,ease:"power2.out"},.64);
    }else{
      if(this.head)tl.to(this.head,{rotation:-4,x:-1,duration:.3,ease:"power2.inOut"},.1)
        .to(this.head,{rotation:3,x:1,duration:.32,ease:"power2.inOut"})
        .to(this.head,{rotation:0,x:0,duration:.28,ease:"power2.out"});
      if(this.torso)tl.to(this.torso,{rotation:1.8,y:1.5,duration:.34,ease:"power2.inOut"},.1).to(this.torso,{rotation:0,y:0,duration:.34,ease:"power2.out"});
    }
    return true;
  }
  jump(onComplete){
    if(!motionGSAP||!this.sprite)return false;
    this.killIdle();this.killWalk();this.killAction();this.actionBusy=true;
    const finish=()=>{this.actionBusy=false;this.actionTl=null;this.neutralParts(.12);this.startIdle();onComplete?.()};
    this.actionTl=motionGSAP.timeline({onComplete:finish});
    if(this.legL)this.actionTl.to(this.legL,{rotation:5,y:4,duration:.12,ease:"power2.in"},0);
    if(this.legR)this.actionTl.to(this.legR,{rotation:-5,y:4,duration:.12,ease:"power2.in"},0);
    if(this.armL)this.actionTl.to(this.armL,{rotation:-9,duration:.12},0);
    if(this.armR)this.actionTl.to(this.armR,{rotation:9,duration:.12},0);
    this.actionTl.to(this.sprite,{y:-108,rotation:-1.2,duration:.34,ease:"power2.out"},.10)
      .to(this.sprite,{y:0,rotation:.5,duration:.34,ease:"power2.in"})
      .to(this.sprite,{rotation:0,duration:.14,ease:"back.out(1.6)"});
    if(this.shadow)motionGSAP.timeline().to(this.shadow,{scaleX:.62,scaleY:.62,opacity:.18,duration:.34,ease:"power2.out"}).to(this.shadow,{scaleX:1,scaleY:1,opacity:.42,duration:.34,ease:"power2.in"});
    return true;
  }
}
...
function syncAuroraWalk(el,velocity,maxVelocity){
  ...
  if(ratio>.035)controller.walk(.78+ratio*.72);else controller.stop();
}
...
function pulseAuroraAction(el,action,duration=1200){
  if(!el)return;
  clearAuroraActions(el);el.classList.add(action);
...
function setAuroraFacing(el,left){
  ...
  el.classList.add("turning");window.setTimeout(()=>el.classList.remove("turning"),240);el.classList.toggle("face-left",left);
}
function ambientAurora(el,moving){
  if(!el||moving)return;
  const now=performance.now();if(now<nextAmbientMotion)return;
  if(el.classList.contains("bend")||el.classList.contains("study-object")||el.classList.contains("reach-object")||el.classList.contains("turning"))return;
  pulseAuroraAction(el,"look-around",1500);nextAmbientMotion=now+5200+Math.random()*4200;
}
...
function show(s){
  ...
  if(journalQuickBtn) journalQuickBtn.hidden=!["game","chamber","desert","worldMap"].includes(s.id);
}
...
resetGameBtn.onclick=()=>{
  storage.remove("aurora_compass"); storage.remove("aurora_compass_v521"); storage.remove("aurora_prologue_seen"); storage.remove("aurora_journal_v580"); storage.remove("aurora_save_v580");
  location.reload();
};
...
const prologueScenes=[
  ...
  },
  {
    ...
    text:"Et brev uten avsender ligger og venter på dørmatta når hun kommer hjem … «HOLD DEG UNNA KONGENES DAL. Noen graver bør forbli uåpnet. Dette er din eneste advarsel.» Nederst er et ukjent symbol. Aurora fotograferer brevet og legger det i feltdagboken."
  },
  {
...
function renderPrologue(){
  ...
  if(scene.interactive==="scan"){
    ...
    prologueNext.textContent=prologueIndex===prologueScenes.length-1?"REIS TIL KONGENES DAL":"FORTSETT";
  }
...
function openPrologue(){
  ...
  storage.remove("aurora_journal_v580");
  storage.remove("aurora_save_v580");
...
function loop(t){
 ...
 if(!state.arrivalLogged && Math.abs(state.x-180)>24) state.arrivalLogged=true;
...
const chamberAurora=$("chamberAurora"),chamberCamera=$("chamberCamera"),innerDoor=$("innerDoor"),cinematicBars=$("cinematicBars"),transitionFade=$("transitionFade"),chamberExitFade=$("chamberExitFade"),inventoryBadge=$("inventoryBadge");
...
let journalReturnScreen=null;
let journalOpenedFromCompass=false;

function journalFlags(){
  return {
    entrance:state.arrivalLogged,
    floor:state.stage>=1,
    arrows:state.stage>=2,
    eye:state.eyeOpened || state.stage>=3,
    inscription:chamberStage>=1,
    inner:chamberStage>=3,
    compass:chamberStage>=4 || compassCollected
  };
}

function updateJournal(){
  const flags=journalFlags();
  const order=["entrance","floor","arrows","eye","inscription","inner","compass"];
  const done=order.filter(k=>flags[k]).length;
  const progressText=$("journalProgressText");
  if(progressText)progressText.textContent=`${done} / ${order.length} SPOR`;

  document.querySelectorAll(".field-node[data-jstep]").forEach(node=>{
    const key=node.dataset.jstep;
    node.classList.toggle("done",!!flags[key]);
    node.classList.toggle("locked",!flags[key]);
  });

  const mapLegend=$("journalMapLegend");
  if(mapLegend){
    mapLegend.textContent=done===order.length
      ? "Kartet over funnstedet er komplett."
      : `${done} av ${order.length} områder eller spor er kartlagt.`;
  }

  const notes=[];
  notes.push([flags.entrance,"Ankomst","Den ukjente koordinaten i Kongenes dal er lokalisert."]);
  notes.push([flags.floor,"Gulvet","En uvanlig slitt stein viser at noe har blitt aktivert her tidligere."]);
  notes.push([flags.arrows,"Retningen","Riss og piler i steinen leder videre mot en skjult mekanisme."]);
  notes.push([flags.eye,"Horus-øyet","Et Horus-øye hugget i veggen reagerer og avslører den skjulte passasjen."]);
  notes.push([flags.inscription,"Innskriften","Tre tegn gjentas: solskiven, Horus-øyet og ankh."]);
  notes.push([flags.inner,"Innerste kammer","Bak portalen ligger et mindre kammer med en forseglet kiste."]);
  notes.push([flags.compass,"Objekt 001","Et gammelt kompass er funnet. Nålen peker ikke mot nord."]);
  const notesEl=$("journalNotes");
  if(notesEl)notesEl.innerHTML=notes.map(([ok,title,text])=>
    `<article class="journal-note ${ok?"done":"locked"}"><i>${ok?"✓":"·"}</i><div><b>${title}</b><p>${ok?text:"Ikke oppdaget ennå."}</p></div></article>`
  ).join("");

  const discovered=flags.compass;
  $("journalFindLocked")?.classList.toggle("hidden",discovered);
  $("journalCompassFind")?.classList.toggle("hidden",!discovered);
  const compassStatus=$("journalCompassStatus");
  if(compassStatus && discovered){
    compassStatus.textContent=compassCollected
      ? "Sikret og registrert for videre undersøkelse."
      : "Lokalisert i den forseglede kisten. Plasseringen dokumenteres før objektet løftes ut.";
  }
}

function openJournal(returnScreen,fromCompass=false){
  journalReturnScreen=returnScreen || screens.find(x=>x.classList.contains("active")) || game;
  journalOpenedFromCompass=fromCompass;
  updateJournal();
  show(journal);
}
...
journalQuickBtn?.addEventListener("click",()=>{
  ...
  if(!active || active===journal)return;
  openJournal(active,false);
});

document.querySelectorAll("[data-jtab]").forEach(button=>{
  button.addEventListener("click",()=>{
    const tab=button.dataset.jtab;
    document.querySelectorAll("[data-jtab]").forEach(b=>b.classList.toggle("active",b===button));
    document.querySelectorAll("[data-jpanel]").forEach(p=>p.classList.toggle("active",p.dataset.jpanel===tab));
  });
});
...
compassArtifact.onclick=()=>{
  ...
  setTimeout(()=>openJournal(chamber,true),450);
};

$("closeJournal").onclick=()=>{
  if(!journalOpenedFromCompass){
    const target=journalReturnScreen || game;
    journalReturnScreen=null;
    show(target);
    return;
  }
  journalOpenedFromCompass=false;
  if(!compassCollected){
...
let desertX=20,desertLeft=false,desertRight=false,desertVelocity=0,desertStarted=false,desertCompletionQueued=false;
...
endingClose?.addEventListener("click",()=>{
  ...
  desert.classList.remove("ending-mode");
  show(worldMap);
  selectMapSite("mycenae");
  setMapMode("regional");
  if(motionGSAP){
    motionGSAP.fromTo(".world-map-panel",{y:28,opacity:0},{y:0,opacity:1,duration:.55,ease:"power3.out"});
    motionGSAP.fromTo(".map-site.unlocked",{scale:.88},{scale:1,duration:.55,ease:"back.out(2)"});
  }
});

const mycenaeNext=$("mycenaeNext"),mapNotice=$("mapNotice");
const mapPanelKicker=$("mapPanelKicker"),mapPanelTitle=$("mapPanelTitle"),mapPanelText=$("mapPanelText");
const geoMapShell=$("geoMapShell"),mapRegionBtn=$("mapRegionBtn"),mapWorldBtn=$("mapWorldBtn");

const siteInfo={
  egypt:{kicker:"FULLFØRT EKSPEDISJON",title:"Kongenes dal · Egypt",text:"Kapittel 1 er fullført. Kompasset ble funnet i det skjulte gravkammeret.",status:"done"},
  mycenae:{kicker:"NESTE EKSPEDISJON",title:"Mykene · Hellas",text:"Kompasset peker videre mot det arkeologiske området Mykene i Argolis. Reisemålet er låst opp.",status:"unlocked"},
  rome:{kicker:"LÅST REISEMÅL",title:"Roma · Italia",text:"Historisk reisemål. Låses opp senere i Aurora Vales reise.",status:"locked"},
  gobekli:{kicker:"LÅST REISEMÅL",title:"Göbekli Tepe · Tyrkia",text:"Det arkeologiske stedet ligger i Sørøst-Anatolia. Låses opp senere.",status:"locked"},
  petra:{kicker:"LÅST REISEMÅL",title:"Petra · Jordan",text:"Den historiske klippebyen i Jordan. Låses opp senere.",status:"locked"},
  machu:{kicker:"LÅST REISEMÅL",title:"Machu Picchu · Peru",text:"Inkaområdet i Andesfjellene. Låses opp senere.",status:"locked"},
  chichen:{kicker:"LÅST REISEMÅL",title:"Chichén Itzá · Mexico",text:"Det store mayaområdet på Yucatán-halvøya. Låses opp senere.",status:"locked"},
  oseberg:{kicker:"LÅST REISEMÅL",title:"Oseberg · Vestfold, Norge",text:"Oseberg er plassert ved funnstedet i Vestfold – ikke ved museet i Oslo. Låses opp senere.",status:"locked"}
};

function selectMapSite(key){
  const info=siteInfo[key];
  if(!info)return;
  if(mapPanelKicker)mapPanelKicker.textContent=info.kicker;
  if(mapPanelTitle)mapPanelTitle.textContent=info.title;
  if(mapPanelText)mapPanelText.textContent=info.text;
  if(mycenaeNext){
    const isMycenae=key==="mycenae";
    mycenaeNext.hidden=!isMycenae;
    if(isMycenae)mycenaeNext.textContent="MYKENE · LÅST OPP";
  }
  document.querySelectorAll(".map-site").forEach(btn=>btn.classList.toggle("selected",btn.dataset.site===key));
}

document.querySelectorAll(".map-site").forEach(btn=>btn.addEventListener("click",()=>selectMapSite(btn.dataset.site)));

function setMapMode(mode){
  if(!geoMapShell)return;
  const regional=mode!=="world";
  geoMapShell.classList.toggle("regional",regional);
  geoMapShell.classList.toggle("world",!regional);
  mapRegionBtn?.classList.toggle("active",regional);
  mapWorldBtn?.classList.toggle("active",!regional);
}
mapRegionBtn?.addEventListener("click",()=>setMapMode("regional"));
mapWorldBtn?.addEventListener("click",()=>setMapMode("world"));
...
function desertLoop(){
  if(desert.classList.contains("active")){
    ...
    if(desertX>70 && !desertCompletionQueued){
      desertCompletionQueued=true;
      desertLeft=false; desertRight=false; desertVelocity=0;
      desertAurora.classList.remove("walk");
      ...
      setTimeout(()=>{
        if(!desert.classList.contains("active"))return;
...
/* v5.7.1 release safeguard */
window.addEventListener("DOMContentLoaded",()=>{
  const endBtn=document.getElementById("endingClose");
  if(endBtn) endBtn.textContent="TILBAKE TIL VERDENSKARTET";
  document.documentElement.dataset.build="5.7.2";
});
